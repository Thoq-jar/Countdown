...
export class AppComponent {
  ...
  async setNotification(inputValue: string) {
    ...
    if (permission === "granted" && !isNaN(inputDateTime.getTime())) {
      ...
      alert("Notification permission was not granted or an error occurred while parsing the input date!");
    }
  }
  ...
  showNotification(title: string, options?: NotificationOptions) {
    if (Notification.permission === "granted") {
      ...
      alert("Notification permission not granted which are required to use countdown!");
    }
  }
  ...
  startCountdown(inputDateTime: Date) {
    this.countdownInterval = setInterval(() => {
      ...
      if (timeDiff <= 0) {
        ...
        this.deliverNotification("Countdown", "Countdown for event has ended!").then(_ => {});
